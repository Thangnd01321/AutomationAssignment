...
public class LoggingAspect {
  ...
  @Before("execution(* utils.logging.ApiAssert+.*(..))")
  public void logBeforeApiAssertion(JoinPoint joinPoint) {
    Object[] args = joinPoint.getArgs();
    StringBuilder builder = new StringBuilder();
    for (Object value : args) {
      if (value == null) {
    ...
    Signature signature = joinPoint.getSignature();
    if (builder.length() == 0) {
      Log.action(signature.getDeclaringTypeName() + "." + signature.getName() + "()");
    } else {
      Log.action(
          signature.getDeclaringTypeName() + "." + signature.getName() + "(" + builder + ")");
    }
    beforeActionTime = LocalDateTime.now();
  }

  @AfterThrowing(pointcut = "execution(* utils.logging.ApiAssert+.*(..))", throwing = "error")
  public void logAfterApiAssertionFailure(JoinPoint joinPoint, Throwable error) throws IOException {
